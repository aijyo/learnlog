...
// Class to implement the RandomNormal operation
class RandomNormalOp {
public:
    // Constructor for the RandomNormalOp class.
    // Parameters:
    // - shape: a vector representing the dimensions of the output tensor.
    ...
    RandomNormalOp(const std::vector<int64_t>& shape, float mean, float stddev, std::string dtype)
        : shape_(shape), mean_(mean), stddev_(stddev), dtype_(dtype) {
    }

    // This method computes the shape of the output tensor.
    // For simplicity, we return the shape as is, as it's already provided.
    ...
    // The distribution is defined by the mean and standard deviation provided during initialization.
    std::vector<float> generate() {
        ...
        for (auto dim : shape_) {
            total_elements *= dim;  // Calculate the total number of elements based on the shape.
        }
    ...
    std::string getRandomNormalElementType() {
        return dtype_;
    }
    ...
    std::vector<std::string> resultTypeInference() {
        return { getRandomNormalElementType() };  // Return the data type for the output tensor.
    }

    // This method prints the generated random tensor.
    void printRandomNormalTensor() {
        auto result = generate();
        std::cout << "Generated Random Normal Tensor: ";
...
private:
    std::vector<int64_t> shape_;  // Shape of the output tensor.
...
// Main function to test the RandomNormalOp class.
int main() {
    std::vector<int64_t> shape = { 2, 3 };  // Define a 2x3 matrix as the output tensor shape.
    ...
    // Create an instance of RandomNormalOp with the above parameters.
    RandomNormalOp randomOp(shape, mean, stddev, dtype);

    // Compute and print the shape of the output tensor.
    ...
    randomOp.printRandomNormalTensor();
