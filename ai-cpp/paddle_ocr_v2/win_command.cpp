...
// Small helpers
// -------------------------------
static std::wstring GetExeDirW_()
{
...
static std::wstring JoinPathW_(const std::wstring& a, const std::wstring& b)
{
    if (a.empty()) return b;
...
static std::string ToUtf8_(const std::wstring& w)
{
    if (w.empty()) return "";
...
static void EnsureDefaultOcrPaths_(std::string& det_dir, std::string& rec_dir, std::string& paddlex_config)
{
    // English comment:
    // If caller didn't provide explicit OCR model paths, default to:
    //   <exe_dir>\model\ocr.yaml
    //   <exe_dir>\model\PP-OCRv5_mobile_det_infer
    //   <exe_dir>\model\en_PP-OCRv4_mobile_rec_infer
    const std::wstring exeDir = GetExeDirW_();
    const std::wstring modelDir = JoinPathW_(exeDir, L"model");
    const std::wstring defDet = JoinPathW_(modelDir, L"PP-OCRv5_mobile_det_infer");
    const std::wstring defRec = JoinPathW_(modelDir, L"en_PP-OCRv4_mobile_rec_infer");
    const std::wstring defYaml = JoinPathW_(modelDir, L"ocr.yaml");

    if (det_dir.empty()) det_dir = ToUtf8_(defDet);
    if (rec_dir.empty()) rec_dir = ToUtf8_(defRec);
    if (paddlex_config.empty()) paddlex_config = ToUtf8_(defYaml);
}
...
namespace wowapp {
    ...
    static bool ParseConfig_(const std::string& json_utf8, RuntimeConfig& out, std::string& err)
    {
        ...
        try {
            ...
            // Apply exe-dir defaults if ocr paths are omitted.
            EnsureDefaultOcrPaths_(out.det_dir, out.rec_dir, out.paddlex_config);
