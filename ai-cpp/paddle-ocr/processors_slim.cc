...
absl::StatusOr<std::vector<cv::Mat>>
DBPostProcess::Apply(std::vector<cv::Mat> &input, const void *param_ptr,
                     const std::vector<int> *img_shapes) const {
  if (input.empty()) {
    return absl::InvalidArgumentError("DBPostProcess::Apply: input is empty");
  }
  if (!param_ptr) {
    return absl::InvalidArgumentError("DBPostProcess::Apply: param_ptr is null (DBPostProcessResult*)");
  }
  if (!img_shapes || img_shapes->size() < 2) {
    return absl::InvalidArgumentError("DBPostProcess::Apply: img_shapes is null or invalid");
  }
  DBPostProcessResult *out = (DBPostProcessResult *)param_ptr;
  out->boxes.clear();
  out->scores.clear();

  auto r = (*this)(input[0], *img_shapes);
  if (!r.ok()) return r.status();
  out->boxes = std::move(r.value().first);
  out->scores = std::move(r.value().second);
  return input;
}
