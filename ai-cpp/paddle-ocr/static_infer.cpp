...
#include "utils/mkldnn_blocklist.h"
...
PaddleInfer::PaddleInfer(const std::string& model_name,
    ...
    for (const auto& name : input_names) {
        ...
        auto shp = handle->shape();
        auto dt = handle->type();
        INFOE("predictor: %s, shape[%d], type[%s]\n", name.c_str(), (int)shp.size(), dt);
...
PaddleInfer::Apply(const std::vector<cv::Mat>& x) {
    for (size_t i = 0; i < x.size(); ++i) {
        ...
        auto data = (float*)x[i].data;
        input_handle->CopyFromCpu<float>(data);
    }
    try {
        predictor_->Run();
    }
