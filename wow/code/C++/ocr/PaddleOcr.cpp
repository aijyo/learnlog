...
namespace {
    ...
    static bool FindPaddleModelFiles(const std::string& model_dir, std::string& model_file, std::string& params_file) {
        ...
        // Common modern export
        fs::path pdmodel = dir / "model.pdmodel";
        fs::path pdiparams = dir / "model.pdiparams";
        if (fs::exists(pdmodel) && fs::exists(pdiparams)) {
            model_file = pdmodel.string();
            params_file = pdiparams.string();
            return true;
        }
...
bool PaddleOcr::CreatePredictors(std::string& err) {
    ...
    impl->dict = LoadDict(cfg_.rec_dict_path);
    if (impl->dict.empty()) {
